...
const PlacementCard = ({ placement, authToken  }) => {
  ...
  const handleApply = async () => {
    try {
      ...
      alert('An error occurred while applying for the placement. Please try again.');
    }
  };
  ...
  return (
    <div className="bg-white max-w-72 shadow-lg min-w-80 p-4 rounded-md font-semibold text-base">
      ...
      <br />
      <div className="font-normal text-xl font-serif">Job Role: {placement.jobTitle}</div>
      ...
      <div className="flex flex-row mt-6">
        {applied ? (
          <button className="bg-green-500 text-white p-2 text-base rounded-md" disabled>
            Already Applied
          </button>
        ) : (
          <button
